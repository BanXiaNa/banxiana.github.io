...
对于大量数据的插入，insert就不适用了，我们可以使用load大量的去加载本地文件的数�?
...
# 设置全局参数 local-infile开关，开启从本地文件导入数据的开�?
set global local_infile = 1;
# 使用load语句导入数据
load data local infile "文件路径/文件�? into table '表名' fields terminated by '字段分隔�?
...
4：业务尽量避免对主键的修改，因为维护索引会造成大量的性能开�?

---

## order by 优化

首先，排序分为两种排序操作：using filesort �?using Index
...
4：不可避免出现filesort时，可以调节缓冲区大小，默认�?56k

---

## group by 优化

好像也没什么注意的，就是要建立联合索引，满足最左前缀法则之类�?
...
可以创建覆盖索引，通过联合查询或者子查询来解决问�?

---

## count优化

这个对于不同的引擎，count的效率也会不�?
**MyISAM** 引擎把一个表的总行数存在了磁盘上，count( * )会直接返�?
**InnoDb** 引擎执行count( * )会把表的所有数据读取出来，然后计数

count原理：count会对结果集进行一步步地判断，如果结果不是null，累计值就加一，最后返回累计�?

count( * )：数据库做了优化，不会取出一行行的数据，所以尽量使�?count( * )
