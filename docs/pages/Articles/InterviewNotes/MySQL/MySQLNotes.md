...
- 读未提交 READ UNCOMMITTED：查询语句可以查询别的事务更改的数据，即使这个事务没有被提交，所以可能读取到错误的数据，完全没有隔离性
- 读已提交 READ COMMITTED：查询可以读取已经提交的事务数据，但是同一个事务两次读取可能不一样，这就是不可重复度，Oracle、PostgreSQL、SQL Server 默认都是这个级别。
- 可重复读 REPEATABLE READ：读取操作可以读取已经提交的事务操作，但是只能读取事务开始时的数据，之后提交的事务是看不到的
- 串行化 SERIALIZABLE：事务排队执行，隔离性最高，并发性没有，快照读也加上共享锁，杜绝所有问题，但是效率直接废了

## MySQL 默认的事务隔离级别是什么？为什么选择这个级别？ 

默认级别是 可重复读 REPEATABLE READ，

早期 MYSQL 中，主从同步使用倒放 Binlog 的方式实现，如果使用 读已提交 的方式就会出现事务记录顺序和执行顺序不一致的问题，因为事务记录是根据提交顺序记录的，而不是执行顺序。

现在 Binlog 是记录的行数据，读已提交也不会出现问题了，但是为了向前兼容，还是使用 可重复读

## 数据库的脏读、不可重复读和幻读分别是什么？ 

脏读：读取到了别的事务还没提交的数据，如果事务回滚，这个数据就是废掉的

不可重复读：同一个事务内，两次查询同一行数据，查询的结果不一样，也就是说，两次查询中间有事务提交了修改数据的操作，强调数据不一样

幻读：同一事务内，两个查询查询到的数据行数变了，也就是说两次查询之间，有事务提交修改了数据行数的操作，强调的是数据行数的变化

## MySQL 中有哪些锁类型？

分类上来说可以从粒度上和模式上来分类

- 粒度：表锁，行锁，表锁锁住整张表，整张表只能有一个人访问，行锁锁住一行数据，一行数据同时只能有一个人访问，一般来说，锁的粒度越小，并发效率越高
- 模式：共享锁，排它锁，共享锁在事务读取记录的时候被获得，多个事务可以重复获得同一个锁，不会冲突，排它锁是在事务涉及修改的操作的时候被获得，只能有一个事务持有这个锁，其他事务需要等待

还有一些连七八糟的辅助锁，比如：

- 表锁加锁的时候要检测每一行有没有被加行锁，这时候有 意向锁 来表示这个表有没有行锁存在，意向锁分为两种，共享意向锁和独占意向锁，这两个锁是在加 共享锁 和 排它锁 的时候创建的，用于告诉新来的事务锁的存在情况，从而不用挨个遍历
- 如果同时执行 DDL 和 DML 的话，表的元数据就会被破坏，使用 元数据锁 来保证这两种操作的隔离，其分为读锁和写锁，读取元数据的操作会获得读锁，读锁同样可以重复获取，互不干涉，修改元数据的操作，比如改变表的结构，这个时候会获得写锁，写锁只能获取一次，并且和读锁互斥，可以保障数据的一致性 
- 还有专门维护索引的锁：记录锁，用于条件查询保护索引的，值得注意的是，如果条件查询直接走的是聚簇索引，这个锁就会把聚簇索引锁住，因为没有对应字段的索引，只能锁聚簇索引了
- 还有专门处理幻读的锁：间隙锁，数据库每条数据之间都有间隙，如果在读已提交的隔离条件下，会出现幻读的情况，也就是两次查询之间有记录被插入，使用间隙锁锁住这条记录之间的空隙，防止数据插入，造成幻读的情况，同一个间隙锁可以被获取多次，因为他们的目的是相同的
- 如果一个间隙被间隙锁锁住了，别的事务想要插入就要使用 意向锁 来表示等待状态，同一个间隙可以有多个意向锁存在，因为他们只是在等待，这个锁的目的并不是为了阻止，而是为了表示自己在等待
- 还有一个锁是用于控制自增量的获取的，叫做：Auto-Inc Lock，原理是在插入一条有自增列的数据的时候，会在表上增加一个这个锁，语句完毕之后，解除这个锁，5.1.22 之后，使用互斥量来进行子增量的分发，性能比 Auto-Inc Lock 要高，因为互斥量不需要等待语句执行完毕就可以被释放
  由于并发插入的情况，基于 statement -based binlog 复制时，自增的值顺序无法把控，可能会导致主从数据不一致。
...
简单来说就是 MYSQL 为了保证主从崩溃恢复一致性所做的操作，

MYSQL Service 层有 Binlog，从库拉取这个日志来恢复数据，InnoDB 层还有 redoLog，主库崩溃之后通过这个日志拉取恢复数据，如果这两个日志不能保证一致性，那么主从崩溃恢复的时候也不能保证一致性

这两个日志更新分为两个阶段

- **Prepare**：redo Log 写入数据之后，通知 Binlog 更新数据，并且将自己的数据更新成 prepare 状态，表示未同步
- **Commit**：Binlog 更新之后，落盘成功，通知 redo Log 更新状态为：commit

恢复数据的时候，主库检查 redo Log 每条数据的状态，prepare 状态并且 Binlog 没有，就代表事务没正常写入，回滚，如果是 commit 状态，但是 Binlog 写入，就代表写入成功 ，事务提交，更改 redo Log 为 Commit

这样可以保证 主从同步的一致性

## MySQL 中如果发生死锁应该如何解决？ 

死锁本质是对锁的竞争和不释放锁，InnoDB 本身就有对死锁的检测，如果发生死锁，他会选择死锁事务中代价最小的进行中断，毕竟代价越小的事务 undo Log 越短，回滚开销越低，另外还设有最大等待时间，一般是 五十秒，如果超过了这个时间，还没有获取到锁，就自动进行回滚

还可以手动干预，直接杀死发生死锁的事务

## MySQL 中如何解决深度分页的问题？ 

深度分页带来的问题是大量查询结果被丢弃，造成大量无效 IO，浪费性能，这些查询结果会占用大量内存，CPU 资源，如果需要回表的话，性能更是差到爆

优化：

- 可以用子查询的方式定位先主键，然后拿主键再查询数据，这样可以预防直接回表带来的性能开销
- 可以使用游标进行定位，每次查询返回当前页的最大 ID，下次查询直接进行定位，不用从头进行扫描，缺点就是不支持跳页，但是做成瀑布流更加合适
- 也可以用 ES 做分页

## 什么是 MySQL 的主从同步机制？它是如何实现的？ 

简单来说：主从同步机制是主库推送给从库自己更新的数据，从库拉取到之后，进行回放，就能保证数据一直

主库将数据的更改写在 Binlog 中，主库会监测 Binlog 是否更新，如果更新了，就通知从库有更新，从库去拉取数据，将数据保存到 Relay Log 中，进行回放，从而保证一致性

关于主从复制下主库的事务提交时机，MYSQL 提供了三种机制：

- 异步复制：Binlog 更新之后立刻提交数据，这种性能虽然高，但是不可靠，一旦主库挂了，从库就丢掉数据了
- 同步复制：从库同步完之后会回复确认，所有从库确认之后才进行事务提交，这种效率低，一般不用
- 半同步复制：这种就是折中的方案，我们设定一个值，如果从库同步的数量高于这个值，就进行事务的提交

## 如何处理 MySQL 的主从同步延迟？ 

主从复制主要过程：主库同步 Binlog，主库 dump 线程推送事件，从库拉取 Binlog，从库写入 Relay Log，SQL 线程重放。这一段只要有一个环节慢了，整体就爆炸，由于网络问题，设备问题，这种情况几乎不可避免，但是可以通过其他方法改善

主要优化方案：

- 关键业务使用主库：对于写后读这种场景的操作，直接使用主库读取，虽然违背了主写从读的原则，但是一般这种场景不高，对主库压力不大
- 延迟感知：针对每一次写入，设置一个时间戳，短时间读取请求走主库，过期后走从库，确保从库反应过来
- 二次查询：走从库没有后就去主库查询，但是可能会被攻击，比如大量访问从库没有的数据，导致请求打到主库
- 设置缓存：刚写入的数据并入缓存，读取走缓存，但是这又涉及缓存一致性的问题，属于是拿问题解决问题
- 也可以升级从库配置，太差了肯定不行
